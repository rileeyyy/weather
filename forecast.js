function getWeatherIconUrl(description, isDay = true, windSpeedMph = 0, temperature = null) {
    ...
    if (lowerDesc.includes('thunderstorm')) {
        ...
        if (lowerDesc.includes('mostly')) {
            if (isHot) {
                iconName = 'Mostly Sunny and Hot.png';
            } else if (isWindy) {
                iconName = 'Sunny and Windy.png';
            } else {
                iconName = 'Mostly Sunny.png';
            }
        ...
        if (isHot) {
            iconName = 'Partly Cloudy and Hot.png';
        } else if (isWindy) {
        ...
        if (isHot && lowerDesc.includes('variably')) {
            iconName = 'Variably Cloudy and Hot.png';
        } else if (isHot) {
            iconName = 'Cloudy and Hot.png';
        } else if (isWindy) {
...
function updateCurrentConditions(data) {
    ...
    if (properties) {
        ...
        const iconUrl = getWeatherIconUrl(desc, isDay, windSpeedMph, typeof tempF === 'number' ? tempF : null);
...
function updateHourlyForecast(data) {
    ...
    if (data && data.properties && data.properties.periods) {
        ...
        periods.forEach(period => {
            ...
            const iconUrl = getWeatherIconUrl(period.shortForecast, isDay, windSpeedMph, period.temperature);
...
function updateExtendedForecastDays(numDays) {
    if (window.forecastData && window.forecastData.properties && window.forecastData.properties.periods) {
        ...
        displayPeriods.forEach((period, index) => {
            ...
            const iconUrl = getWeatherIconUrl(period.shortForecast, period.isDaytime, windSpeedMph, period.temperature);
